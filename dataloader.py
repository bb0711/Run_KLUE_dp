...
class KlueDpDataLoader(object):
    ...
    # using predicted head, change data loader's head
    def change_test_dataloader(self, head, data_filename: str = "klue-dp-v1_test.tsv", **kwargs):
        dataset = self.dataset.change_test_dataset(head, self.data_dir, data_filename)
        return DataLoader(
            dataset,
            batch_size=self.args.eval_batch_size,
            shuffle=False,
