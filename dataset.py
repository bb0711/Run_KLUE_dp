...
class KlueDpDataset:
    ...
    def _change_dataset(self, file_path:str, head_preds:List[int]) -> Dataset:
        examples = self._create_examples(file_path)
        
        # added by whj
        #print(len(examples), len(head_preds))
        assert len(examples) == len(head_preds)
        for idx,example in enumerate(examples):
            example.head = head_preds[idx]
    ...
    def change_test_dataset(
        self, head: list, data_dir: str, data_filename: str = "klue-dp-v1_test.tsv"
    ) -> TensorDataset:
        file_path = os.path.join(data_dir, data_filename)
        return self._change_dataset(file_path,head)
