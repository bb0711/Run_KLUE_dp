...
# added by whj
@torch.no_grad()
def pre_inference(data_dir, model_dir, output_dir, args):
    # device setup
    ...
    head_preds, _, _, _ = flatten_prediction_and_labels(predictions, labels)
    return head_preds


@torch.no_grad()
def inference(data_dir, model_dir, output_dir, args, head_preds):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_dir)
    klue_dp_dataset = KlueDpDataLoader(args, tokenizer, data_dir)
    klue_dp_test_loader = klue_dp_dataset.change_test_dataloader( ## whj changed
        head_preds, args.test_filename, **kwargs 
    )
    ...
    write_output(klue_dp_dataset, head_preds, type_preds, data_dir) ## whj added

def write_output(klue_dp_dataset, head_preds, type_preds, data_dir):
    file_path = os.path.join(data_dir, args.test_filename)
    dataset = klue_dp_dataset.dataset._create_examples(file_path)
    KLUE_DP_OUTPUT_TEXT = 'output.txt'
    sent_id = -1
    with open(os.path.join(output_dir, KLUE_DP_OUTPUT_TEXT), "w", encoding="utf8") as f:
        ...
        for idx, ex in enumerate(dataset):
            if sent_id != ex.sent_id:
                f.write("## {}\t{}\n".format(ex.sent_id, ex.text))
                sent_id +=1
            f.write("{}\t{}\t{}\t{}\t{}\n".format(ex.token_id, ex.token, ex.pos, head_preds[idx], DP_LABELS[int(type_preds[idx])]))
...
if __name__ == "__main__":
    ...
    head_preds = pre_inference(data_dir, model_dir, output_dir, args) # added by whj
    inference(data_dir, model_dir, output_dir, args, head_preds)
